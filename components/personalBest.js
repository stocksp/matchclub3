...
const PersonalBest = (props) => {
    ...
    const getBests = async () => {
        try {
            const response = await fetch(`/api/getData?name=getBests&dateId=${props.dateId}`)
    ...
    if (props.highScores && props.highScores.dateResults.length > 0 && bests.length > 0) {
